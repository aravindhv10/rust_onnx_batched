...
*** COMMENT Important apt install stuff
Install basic apt packages.
#+begin_src conf :tangle ./Dockerfile
  RUN \
      --mount=target=/var/lib/apt/lists,type=cache,sharing=locked \
...
*** COMMENT Get onnx rt requirements
Install basic apt packages.
#+begin_src conf :tangle ./Dockerfile
  RUN \
...
*** COMMENT build and install onnxruntime
Install basic apt packages.
#+begin_src conf :tangle ./Dockerfile
  RUN \
      echo 'START build and install onnxruntime' \
...
*** COMMENT Copy the installed libraries
#+begin_src conf :tangle ./Dockerfile
  RUN \
...
* Commit the changes
#+begin_src sh :shebang #!/bin/sh :results output :tangle ./.git.sh
  GITCOMMIT 'Trying onnx rocm image again'
#+end_src
